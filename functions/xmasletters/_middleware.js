  ...
  try{
    ...
    if(request.method==='POST'){
      ...
      if(submittedPassword===storedPassword){
        ...
        return new Response(gateHTML(pageId,true),{status:401,headers:{'Content-Type':'text/html; charset=utf-8'}});
      }
    }
    const cookies=request.headers.get('Cookie')||'';
    const cookieMatch=cookies.split(';').some(c=>c.trim()===`${cookieName}=${expectedCookieValue}`);
    if(cookieMatch)return next();
    return new Response(gateHTML(pageId,false),{status:200,headers:{'Content-Type':'text/html; charset=utf-8'}});
  }catch(e){
    return next();
  }
}
